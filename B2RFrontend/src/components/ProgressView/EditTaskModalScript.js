...
export default {
    ...
    methods: {
        ...
        onPhotoSelected(event) {
            ...
            this.ModifiedState.NewPhotos = Array.from(
                this.ModifiedState.NewPhotos.concat(photos)
            );
        },
        deletePhotoFromExisting(index) {
            this.ModifiedState.DeletedPhotos.push(
                this.ModifiedState.Photos[index]
            );
        ...
        onCADFileSelected(event) {
            ...
            this.ModifiedState.NewCADFiles = Array.from(
                this.ModifiedState.NewCADFiles.concat(CADFiles)
            );
        },
        deleteFileFromExisting(index) {
            this.ModifiedState.DeletedCADFiles.push(
                this.ModifiedState.CADFiles[index]
            );
        ...
        deleteTask() {
            ...
            deleteDoc(
                doc(
                    db,
                    "ProgressViewTasks/" + this.CatID + "/" + this.Status,
                    this.data.ID
                )
            )
                .then(() => {
        ...
        async handleSubmit() {
            ...
            const cadPromises = Array.from(this.ModifiedState.NewCADFiles).map(
                async (file) => {
            ...
            const photoPromises = Array.from(this.ModifiedState.NewPhotos).map(
                async (file) => {
                    let randomString = Math.random()
                        .toString(10)
                        .substring(2, 7);
            ...
            if (this.ModifiedState.Component != this.Component) {
                ...
                if (this.ModifiedState.categoryId != "") {
                    await setDoc(
                        ...
                        ),
                        {
                            ...
                            Photos: this.ModifiedState.Photos.concat(
                                photoDownloadUrls
                            ),
                            CADFiles:
                                this.ModifiedState.CADFiles.concat(
                                    cadDownloadUrls
                                ),
                            id: this.data.ID,
                        ...
                        deleteDoc(
                            ...
                            .catch((error) => {
                                ...
                                this.toast.error(
                                    "An Error Occurred When Moving The Task",
                                    {
                                        timeout: 10000,
                                        position: POSITION.BOTTOM_RIGHT,
                                    }
                ...
                await updateDoc(doc(documentObject, this.data.ID), {
                    ...
                    CADFiles:
                        this.ModifiedState.CADFiles.concat(cadDownloadUrls),
                })
                    .then(() => {
