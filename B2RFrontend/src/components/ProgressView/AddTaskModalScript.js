...
export default {
    ...
    methods: {
        ...
        async handleSubmit() {
            ...
            const q = query(
                documentCollection,
                where("ComponentName", "==", this.Component)
            );
            ...
            if (querySnapshot.empty) {
                console.log(
                    "Error, component " + this.Component + " does not exist"
                );
            } else {
                categoryId = querySnapshot.docs[0].id;
            }
            if (categoryId != "") {
                const cadPromises = Array.from(this.CADFiles).map(
                    async (file) => {
                ...
                const photoPromises = Array.from(this.Photos).map(
                    async (file) => {
                        let randomString = Math.random()
                            .toString(10)
                            .substring(2, 7);
                ...
                await addDoc(
                    collection(db, "ProgressViewTasks/" + categoryId + "/ToDo"),
                    {
