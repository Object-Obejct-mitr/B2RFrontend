<template>
  <a class="btn btn-sm btn-primary me-2" data-mdb-toggle="modal" :data-mdb-target="'#editModal' + id"><i
      class="fas fa-pencil-alt"></i></a>
  <div class="modal modal-lg fade nonGrab container-fluid" data-mdb-backdrop="static" :id="'editModal' + id" tabindex="-1"
    role="dialog">
        ...
        <form id="editContactForm" @submit.prevent="editContact">
          ...
          <div class="modal-body">
            <div class="d-flex flex-column">
              <InputText notchWidth="19ch" id="firstName" label="First Name/Company Name"
                v-model="editedContact.firstName" :isRequired="true" class="mt-3 me-2 w-100" />
              <InputText notchWidth="8ch" id="lastName" label="Last Name" v-model="editedContact.surName"
                :isRequired="false" class="mt-3 me-2 w-100" />

              <InputPhoneNumber notchWidth="11ch" id="phoneNumber" label="Phone Number"
                v-model="editedContact.phoneNumber" :isRequired="false" class="mt-3 me-2 w-100" />

              <InputEmail notchWidth="5ch" id="email" label="Email" v-model="editedContact.email" :isRequired="false"
                class="mt-3 me-2 w-100" />
              <InputChips notchWidth="18ch" id="tags" label="Tags (Comma Separated)" v-model="editedContact.tags"
                :isRequired="false" class="mt-3 me-2 w-100" />

              <InputTextArea id="description" :numRows="6" v-model="editedContact.description" label="Description"
                notchWidth="8ch" class="mt-3" />

              <div class="form-outline mt-3 w-100">
                <i class="fas fa-angle-down trailing"></i>

                <select class="form-control active" id="category" v-model="editedContact.category" required>
          ...
          <div class="modal-footer">
            ...
            <button type="submit" :id="'editContactSubmit' + id" class="btn btn-primary">Save Changes</button>
          </div>
        </form>
      </div>
    </div>
...
export default {
  ...
  methods: {
    clearInputs() {
      this.editedContact = JSON.parse(JSON.stringify({
    ...
    async editContact() {
      $("#editContactSubmit" + this.id).prepend("<span id=\"submitSpinner\" class=\"spinner-border spinner-border-sm\" role=\"status\" aria-hidden=\"true\">");
      ...
      if (this.editedContact.original_category == this.editedContact.category) {
        ...
        await updateDoc(doc(db, "contactPage/" + categoryId + "/contactsList/" + user_dataID), this.editedContact).then(() => {
  ...
  data() {
    return {
      ...
      description: this.user.description,
      category: this.user.category,
      editedContact: JSON.parse(JSON.stringify({ //Deep copy
        userID: this.user.userID,
        firstName: this.user.firstName,
        surName: this.user.surName,
        phoneNumber: this.user.phoneNumber,
